...
// Use auth UID when available (better than a random ID)
let userId = 'user-' + Date.now() + '-' + Math.floor(Math.random() * 10000);

firebase.auth().signInAnonymously()
  .then((cred) => {
    console.log("User signed in anonymously");
    if (cred?.user?.uid) userId = cred.user.uid;
  })
  .catch((error) => {
    console.error("Error during anonymous authentication: ", error);
  });

// === Write to the variable1 collection ===
const COLLECTION_NAME = "variable3";

function logInteraction(eventType, details = {}) {
  // Ensure details is JSON-serializable (no functions/DOM nodes)
  ...
  .then(() => {
    console.log("Interaction logged to 'variable1' successfully!");
  })
  .catch((error) => {
...
function updateMemory() {
  // 1) Update variable values based on current steps
  variables.a = variables.b = variables.c = variables.temp = null;
  for (let i = 0; i <= currentStep; i++) {
    steps[i].changes.forEach(change => {
      variables[change.variable] =
        change.value === 'a' ? variables.a :
        change.value === 'b' ? variables.b :
        change.value === 'c' ? variables.c :
        change.value === 'temp' ? variables.temp :
        change.value;
    });
  }

  // 2) Update step info + code line highlight
  const info = document.getElementById('step-info');
  if (info) info.textContent = `Step ${currentStep + 1}`;

  steps.forEach(step => {
    const el = document.getElementById(step.line);
    if (el) el.classList.remove('highlight');
  });
  const cur = document.getElementById(steps[currentStep].line);
  if (cur) cur.classList.add('highlight');

  // 3) (no memory SVG drawing anymore)
}
