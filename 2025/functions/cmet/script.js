...
const dbNew = firebase.firestore().collection('functions3');  // Using new collection
...
function toggleStepInto() {
    ...
    // Update label & ARIA pressed state
    if (isSteppingIntoFunction) {
        stepIntoButton.innerHTML = '🙈 Hide Function <span class="pill">Less detail</span>';
        stepIntoButton.setAttribute('aria-pressed', 'true');
    } else {
        stepIntoButton.innerHTML = '🔍 Reveal Function <span class="pill">More detail</span>';
        stepIntoButton.setAttribute('aria-pressed', 'false');
    }

    // Stop the initial pulse once students interact or toggle
    stepIntoButton.classList.remove('attention');
    ...
    // (Optional) log it
    logInteraction('toggleStepInto', { isSteppingIntoFunction });
}
...
window.onload = () => {
    ...
    const btn = document.getElementById('step-into-btn');

    // Stop pulsing once student notices it
    btn.addEventListener('mouseenter', () => btn.classList.remove('attention'), { once: true });
    btn.addEventListener('focus',     () => btn.classList.remove('attention'), { once: true });

    // Keyboard shortcut: press "R" to toggle reveal/hide
